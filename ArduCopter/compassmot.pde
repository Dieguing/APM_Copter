...
static uint8_t mavlink_compassmot(mavlink_channel_t chan)
{
    ...
    if (!g.compass_enabled) {
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("compass disabled\n"));
    ...
    for (uint8_t i=0; i<compass.get_count(); i++) {
        if (!compass.healthy(i)) {
            gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("check compass"));
            ap.compass_mot = false;
            return 1;
        }
    }
    ...
    if (!ap.pre_arm_rc_check) {
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("RC not calibrated"));
        ap.compass_mot = false;
        return 1;
    }
    ...
    if (g.rc_3.control_in != 0) {
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("thr not zero"));
        ap.compass_mot = false;
        return 1;
    }
    ...
    // check we are landed
    if (!ap.land_complete) {
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("Not landed"));
        ap.compass_mot = false;
        return 1;
    }
    ...
    gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("STARTING CALIBRATION"));

    // inform what type of compensation we are attempting
    if (comp_type == AP_COMPASS_MOT_COMP_CURRENT) {
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("CURRENT"));
    } else{
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("THROTTLE"));
    }
    ...
    if (updated) {
        ...
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("Calibration Successful!"));
    } else {
        // compensation vector never updated, report failure
        gcs[chan].send_text_P(SEVERITY_HIGH, PSTR("Failed!"));
        compass.motor_compensation_type(AP_COMPASS_MOT_COMP_DISABLED);
    }
